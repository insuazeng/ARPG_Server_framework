...
--Date
--此文件由[BabeLua]插件自动生成
module("SceneScript.network.worldpacket_handle_dispatcher", package.seeall)
...
PacketDispatcher.unregisterPacketHandler = function (msg_id)
    ...
    Player.unRegScriptHandlePacketOpcode(msg_id)
end
...
PacketDispatcher.packetDispatcher = function (msg_id, guid, msg, msg_len)
    ...
    -- LuaLog.outDebug(string.format("msg len=%u, type=%s, type pbc=%s", msg_len, type(msg), type(gLibPBC)))
